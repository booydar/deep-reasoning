...
# Import GRPOTrainer and GRPOConfig from trl or your custom implementation
from trl import GRPOTrainer, GRPOConfig

from modeling_rmt.experimental import RMTConfig, RMT
...
if __name__ == '__main__':
    ...
    def collate_fn(batch):
        ...
        for sample in batch:
            ...
            if mode == 'masked_latent':
                ...
                for segment in cot_segment_tokens[:-1]:
                    segments.append(make_segment(bos + segment + think, loss=True))
                    segments.append(make_segment(bos + latent_tokens + think, loss=False))
                ...
                segments.append(make_segment(bos + all_cot, loss=False))
                segments.append(make_segment(bos + cot_segment_tokens[-1] + ans, loss=True))
                segments.append(make_segment(bos + labels_tokens + eos, loss=True))
                segments_batch.append(segments)
                continue
            ...
            elif mode == 'first_seg_always':
                first_seg = cot_segment_tokens[0]
                for segment in cot_segment_tokens[:-1]:
                    segments.append(make_segment(bos + first_seg + segment + think, loss=True))
                segments.append(make_segment(bos + first_seg + cot_segment_tokens[-1] + ans, loss=True))
                segments.append(make_segment(bos + first_seg + labels_tokens + eos, loss=True))
                segments_batch.append(segments)
                continue

            elif mode == 'overlap_and_first':
                first_seg = cot_segment_tokens[0]
                for i, segment in enumerate(cot_segment_tokens[1: -1]):
                    segments.append(make_segment(bos + first_seg + cot_segment_tokens[i-1] + think + segment + think, loss=True))
                if len(cot_segment_tokens) >= 2:
                    segments.append(make_segment(bos + first_seg + cot_segment_tokens[-2] + think + cot_segment_tokens[-1] + ans, loss=True))
                segments.append(make_segment(bos + first_seg + cot_segment_tokens[-1] + ans + labels_tokens + eos, loss=True))
                segments_batch.append(segments)
                continue
    ...
    def compute_exact_match_reward(samples, predictions, tokenizer):
        """
        Args:
            samples: list of dicts, each with 'labels' (string or token ids)
            predictions: list of generated strings (or token ids)
            tokenizer: tokenizer to decode if needed
        Returns:
            rewards: list of floats (1.0 for exact match, 0.0 otherwise)
        """
        rewards = []
        for sample, pred in zip(samples, predictions):
            # If predictions are token ids, decode
            if isinstance(pred, (list, np.ndarray, torch.Tensor)):
                pred_str = tokenizer.decode(pred, skip_special_tokens=True)
            else:
                pred_str = pred
            # Get reference string
            ref = sample['labels']
            if isinstance(ref, (list, np.ndarray, torch.Tensor)):
                ref_str = tokenizer.decode(ref, skip_special_tokens=True)
            else:
                ref_str = ref
            rewards.append(float(pred_str.strip() == ref_str.strip()))
        return rewards

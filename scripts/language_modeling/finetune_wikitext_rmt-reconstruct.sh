...
MAIN_SCRIPT="${SCRIPT_DIR}/run_finetuning_lm-reconstruct_rmt.py"
...
accelerate launch --num_processes $NP --config_file $ACCEL_CONFIG --main_process_port 29002 $MAIN_SCRIPT \
        ...
        --output_dir ${RUNS_DIR}/${DATASET_NAME}-reconstruct/$MODEL_NAME/${SCHEDULER}_adamw_wd1e-03_${MAX_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_bptt-${K2}/run_$N \
